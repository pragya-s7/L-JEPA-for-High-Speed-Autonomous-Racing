...
def compute_reward(prev_pose, curr_pose, collision, reward_cfg):
    ...
    # Raw distance traveled (always positive, no heading bias)
    distance = np.sqrt(dx*dx + dy*dy)
    
    # Heading change penalty — discourages turning in circles
    heading_change = abs(curr_pose[2] - prev_pose[2])
    # Wrap to [-pi, pi]
    heading_change = abs((heading_change + np.pi) % (2 * np.pi) - np.pi)
    
    r = reward_cfg['progress_weight'] * distance
    r -= reward_cfg.get('heading_penalty', 0.5) * heading_change
